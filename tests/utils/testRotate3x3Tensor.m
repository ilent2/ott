function tests = testRotate3x3Tensor
...
function testFunction(testCase)
  ...
  A1 = ott.utils.rotate_3x3tensor(alpha, rotation);
  testCase.verifyThat(size(A1), IsEqualTo(size(alpha)), ...
      'Incorrect size for matrix A1');

  A3 = ott.utils.rotate_3x3tensor(alpha, 'sphdirection', [0.1; 0.5]);
...
function testDirection(testCase)
  ...
  result = diag([1, 3, 1]);
  direction = [0; 1; 0];
  A = ott.utils.rotate_3x3tensor(alpha, 'dir', direction);
