classdef Empty < ott.shape.Shape
...
% using/distributing this file
  ...
  methods (Hidden)
    ...
    function [locs, norms, dist] = intersectAllInternal(~, x0, ~)
      % All nans
      locs = nan(3, size(x0, 2));
      norms = nan(3, size(x0, 2));
      dist = nan(1, size(x0, 2));
    end

    function [locs, norms] = intersectInternal(~, x0, ~)
      % All nans
      locs = nan(3, size(x0, 2));
      norms = nan(3, size(x0, 2));
    end
    ...
    function S = surfInternal(~, varargin)
      ...
      cube = ott.shape.Cube(p.Results.scale);
