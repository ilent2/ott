  ...
  methods (Hidden, Static)
      ...
      for ii = 1:n_dipoles        % Loop over each dipole
        ...
        for jj = 1:z_rotation     % duplicates for z mirror symmetry
          ...
          for kk = 1:z_mirror     % duplicates for z rotational symmetry
            ...
            A(:, :, kk, ii) = A(:, :, kk, ii) - ...
                ott.scat.vswf.Dda.dipole_nearfield(...
      ...
      for ii = 1:n_dipoles        % Loop over each dipole
        ...
        for jj = 1:z_rotation     % duplicates for z mirror symmetry
          ...
          for kk = 1:z_mirror     % duplicates for z rotational symmetry
            ...
            % Calculate columns of A
            A(:, :, kk, jj, ii) = -ott.scat.vswf.Dda.dipole_nearfield(...
    ...
    function MN = calculate_farfield_modes(theta, phi, Nmax)
      ...
      for ii = 1:numel(theta)
        MN((1:3) + (ii-1)*3, :) = ...
            ott.scat.vswf.Dda.sph2cart_mat(theta(ii), phi(ii)) ...
            * MN((1:3) + (ii-1)*3, :);
      end
    end
    ...
    function data = calc_tmatrix(Nmax, xyz, rtp, modes, alpha, ...
      ...
      [theta, phi] = ott.scat.vswf.Dda.angulargrid(Nmax, z_mirror, z_rotation);
      ...
      if use_nearfield
        ...
        field_func = @ott.scat.vswf.Dda.dipole_nearfield;
      else
        r_near = 1;   % Parameter largely ignored

        MN = ott.scat.vswf.Dda.calculate_farfield_modes(theta, phi, Nmax)./k;
        field_func = @ott.scat.vswf.Dda.dipole_farfield;
      end

      % Pre-calculate inv-alpha
      inv_alpha = ott.scat.vswf.Dda.alpha_to_full_inv_alpha(...
          alpha, size(rtp, 1));
      ...
      if ~low_memory
        ...
        % Pre-calculate A
        A_total = ott.scat.vswf.Dda.interaction_A_total(k, xyz, inv_alpha, ...
      ...
      for m = unique(mmodes).'
        ...
        if low_memory
          ...
          % Pre-calculate A
          A_total = ott.scat.vswf.Dda.interaction_A_lowmem(...
              k, xyz, inv_alpha, z_mirror, z_rotation_safe, m);

          % Pre-calculate F
          F_total = ott.scat.vswf.Dda.nearfield_matrix_lowmem(...
              theta, phi, r_near, k, xyz, rtp, z_mirror, ...
              z_rotation_safe, m, field_func);
        end
        ...
        if ~use_iterative
          ...
          if z_mirror
            ...
            E_odd = ott.scat.vswf.Dda.solve_and_evaluate(Aodd, Fodd, ...
                [Ei_TE(:, evn), Ei_TM(:, ~evn)], use_iterative);
            E_evn = ott.scat.vswf.Dda.solve_and_evaluate(Aeven, Feven, ...
            ...
            E_EM = ott.scat.vswf.Dda.solve_and_evaluate(Aeven, Feven, ...
        ...
        for n = ournmodes
          ...
          if ~use_iterative
            ...
            if z_mirror
              if mod(m + n, 2) == 0
                E_TE = ott.scat.vswf.Dda.solve_and_evaluate(...
                    Aodd, Fodd, Ei_TE(:), use_iterative);
                E_TM = ott.scat.vswf.Dda.solve_and_evaluate(...
                    Aeven, Feven, Ei_TM(:), use_iterative);
              else
                E_TE = ott.scat.vswf.Dda.solve_and_evaluate(...
                    Aeven, Feven, Ei_TE(:), use_iterative);
                E_TM = ott.scat.vswf.Dda.solve_and_evaluate(...
                    Aodd, Fodd, Ei_TM(:), use_iterative);
              end
            else
              % Solve both at the same time (to save time)
              E_EM = ott.scat.vswf.Dda.solve_and_evaluate(Aeven, Feven, ...
