...
%
% Supported casts
%   - ott.shapes.Shape -- Retrieves the geometry (see `shape`)
...
% See LICENSE.md for information about using/distributing this file.
  ...
  methods
    function drag = StokesStarShaped(varargin)
      ...
      %
      % Parameters
      %   - shape (ott.shapes.Shape) -- A shape object with a starShaped
  ...
  methods % Getters/setters
    function drag = set.shape(drag, val)
      assert(isa(val, 'ott.shapes.Shape') && val.starShaped, ...
          'shape must be a star-shaped ott.shapes.Shape instance');
      drag.shape = val;
    end
  end
end
