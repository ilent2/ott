classdef PadeSphere < ott.drag.StokesSphere
...
% See LICENSE.md for information about using/distributing this file.
  ...
  methods
    function obj = PadeSphere(radius, separation, varargin)
      ...
      fxx = sum(table9(:, 2).*as.^table9(:, 1)) ...
          ./ sum(table9(:, 3).*as.^table9(:, 1));
      cyx = sum(table10(:, 2).*as.^table10(:, 1)) ...
          ./ sum(table10(:, 3).*as.^table10(:, 1));
      cyy = sum(table12(:, 2).*as.^table12(:, 1)) ...
          ./ sum(table12(:, 3).*as.^table12(:, 1));
