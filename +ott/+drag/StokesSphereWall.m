...
%
% Supported casts
%   - ott.shapes.Shape -- Cast to plane and sphere
...
% See LICENSE.md for information about using/distributing this file.
  ...
  methods (Static)
    function drag = FromShape(shape, varargin)
      ...
      %
      % Parameters
      %   - shape (ott.shapes.Shape) -- Array of shapes.  Must be two
      ...
      if isa(shape(1), 'ott.shapes.Plane')
        plane = shape(1);
        particle = shape(2);
      else
        plane = shape(2);
      ...
      if ~isa(particle, 'ott.shapes.Sphere')
  ...
  methods
    ...
    function shape = ott.shapes.Shape(drag)
      ...
      sph = ott.shapes.Sphere(drag.radius, 'rotation', drag.rotation);
      pln = ott.shapes.Plane('position', [0;0;1].*drag.separation);
      shape = [sph, pln];
    end
  end
