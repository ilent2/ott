...
% See LICENSE.md for information about using/distributing this file.
  ...
  methods
    function [Texternal, Tinternal, data] = Ebcm(varargin)
      ...
      rM=Texternal.normals(1,:);
      thetaM=Texternal.normals(2,:);
      iKr=ikr;
      iKr_=ikr_;
      dS=Texternal.areas;
        ...
        for kk=1:Nmax
          ...
          if ~(Texternal.xySymmetry && (rem(jj,2)==rem(kk,2)))
            J11(jindx) = mfac.*(pi2*Nm(jj,kk)* ...
                sum(dS.*rM.*jh_.*(YtYp-YpYt),2));
            J22(jindx) = mfac.*(pi2*Nm(jj,kk)* ...
                sum(dS.*(rM.*djdh_.*(YtYp-YpYt) ...
                -thetaM.*(jj*(jj+1)*iKr.* ...
                jdh_.*YYp - kk*(kk+1)*iKr_.* ...
                djh_.*YpY)),2));

            if calcExternalData
              RgJ11(jindx)=mfac.*(pi2*Nm(jj,kk)*sum(dS.* ...
                  rM.*jj_.*(YtYp-YpYt),2));
              RgJ22(jindx)=mfac.*(pi2*Nm(jj,kk)*sum(dS.* ...
                  (rM.*djdj_.*(YtYp-YpYt) ...
                  -thetaM.*(jj*(jj+1)*iKr.* ...
                  jdj_.*YYp - kk*(kk+1)*iKr_.* ...
                  djj_.*YpY)),2));
            end
          end

          if ~(Texternal.xySymmetry && (rem(jj,2)~=rem(kk,2)))
            J12(jindx) = mfac.*(pi2*Nm(jj,kk)*sum(dS.* ...
                (rM.*jdh_.*(YpYp+YtYt) ...
                -kk*(kk+1)*iKr_.*thetaM.* ...
                jh_.*YtY),2));
            J21(jindx) = mfac.*(pi2*Nm(jj,kk)*sum(dS.* ...
                (-rM.*djh_.*(YtYt+YpYp) ...
                +jj*(jj+1)*iKr.*thetaM.*jh_.*YYt),2));

            if calcExternalData
              RgJ12(jindx)=mfac.*(pi2*Nm(jj,kk)*sum(dS.* ...
                  (rM.*jdj_.*(YpYp+YtYt) ...
                  -kk*(kk+1)*iKr_.*thetaM.*jj_.*YtY),2));
              RgJ21(jindx)=mfac.*(pi2*Nm(jj,kk)*sum(dS.* ...
                  (-rM.*djj_.*(YtYt+YpYp) ...
                  +jj*(jj+1)*iKr.*thetaM.*jj_.*YYt),2));
            end
          end
