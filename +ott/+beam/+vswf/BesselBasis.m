...
% See LICENSE.md for information about using/distributing this file.
  ...
  methods (Static)
    ...
    function [bsc, weights] = MathieuBeam(varargin)
      ...
      switch p.Results.parity
        case 'odd'
          A = ott.beam.vswf.Mathieu(phi, morder, ellip, 'se');
        case 'even'
          A = ott.beam.vswf.Mathieu(phi, morder, ellip, 'ce');
        otherwise
      ...
      bsc = ott.beam.vswf.BesselBasis('lmode', lmode, ...
          'angle', p.Results.angle, 'Nmax', p.Results.Nmax, ...
          unmatched{:});
    ...
    function [bsc, weights] = WebberBeam(varargin)
      % Construct a Webber beam using Bessel point-matching
      %
      % Usage
