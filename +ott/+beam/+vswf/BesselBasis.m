    ...
    function beam = BesselBasis(varargin)
      ...
      elseif isempty(p.Results.Etheta) && isempty(p.Results.Ephi)
        ...
        if isempty(polarisation)
          polarisation = repelem([1, 0], numel(theta), 1);
        end
      ...
      beam.basis = 'regular';
      beam = beam.setWavenumber(p.Results.k_medium, 'frequency');
      beam.theta = theta.';
      beam.lmode = p.Results.lmode.';

      % Make the beam vector and store the coefficients
      beam = beam.setCoefficients(sparse(a), sparse(b));
    end
  end
end
