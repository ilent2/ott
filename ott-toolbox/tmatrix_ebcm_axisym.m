...
if verbose
    [nxyz,xyz]=rtpv2xyzv(n,rtp);
    figure
    plot(xyz(:,1).',xyz(:,3).','b','linewidth',2)
    hold on
    plot([xyz(:,1).';xyz(:,1).'+nxyz(:,1).'],[xyz(:,3).';xyz(:,3).'+nxyz(:,3).']);
    hold off
    axis equal
end
...
for jj=1:Nmax
    ...
    for kk=1:Nmax
        ...
        if ~(mirrorsym&&(rem(jj,2)==rem(kk,2)))
            J11(jindx) = mfac.*(pi2*Nm(jj,kk)*sum(dS(:,els).*rM(:,els).*jh_(:,els).*(YtYp-YpYt),1));
            J22(jindx) = mfac.*(pi2*Nm(jj,kk)*sum(dS(:,els).*(rM(:,els).*djdh_(:,els).*(YtYp-YpYt) ...
                -thetaM(:,els).*(jj*(jj+1)*iKr(:,els).*jdh_(:,els).*YYp - kk*(kk+1)*iKr_(:,els).*djh_(:,els).*YpY)),1));
            
            RgJ11(jindx)=mfac.*(pi2*Nm(jj,kk)*sum(dS(:,els).*rM(:,els).*jj_(:,els).*(YtYp-YpYt),1));
            RgJ22(jindx)=mfac.*(pi2*Nm(jj,kk)*sum(dS(:,els).*(rM(:,els).*djdj_(:,els).*(YtYp-YpYt) ...
                -thetaM(:,els).*(jj*(jj+1)*iKr(:,els).*jdj_(:,els).*YYp - kk*(kk+1)*iKr_(:,els).*djj_(:,els).*YpY)),1));
        end
        
        if ~(mirrorsym&&(rem(jj,2)~=rem(kk,2)))
            J12(jindx) = mfac.*(pi2*Nm(jj,kk)*sum(dS(:,els).*(rM(:,els).*jdh_(:,els).*(YpYp+YtYt) ...
                -kk*(kk+1)*iKr_(:,els).*thetaM(:,els).*jh_(:,els).*YtY),1));
            J21(jindx) = mfac.*(pi2*Nm(jj,kk)*sum(dS(:,els).*(-rM(:,els).*djh_(:,els).*(YtYt+YpYp) ...
                +jj*(jj+1)*iKr(:,els).*thetaM(:,els).*jh_(:,els).*YYt),1));
            
            RgJ12(jindx)=mfac.*(pi2*Nm(jj,kk)*sum(dS(:,els).*(rM(:,els).*jdj_(:,els).*(YpYp+YtYt) ...
                -kk*(kk+1)*iKr_(:,els).*thetaM(:,els).*jj_(:,els).*YtY),1));
            RgJ21(jindx)=mfac.*(pi2*Nm(jj,kk)*sum(dS(:,els).*(-rM(:,els).*djj_(:,els).*(YtYt+YpYp) ...
...
RgQv(lengthJs+1:lengthJs*2)=1i*k_medium*(k_particle*RgJ11+k_medium*RgJ22);
RgQv(2*lengthJs+1:lengthJs*3)=1i*k_medium*(k_particle*RgJ22+k_medium*RgJ11);
...
i_s=[i_indexes;i_indexes;i_indexes+Nmax*(Nmax+2);i_indexes+Nmax*(Nmax+2)];
j_s=[j_indexes;j_indexes+Nmax*(Nmax+2);j_indexes;j_indexes+Nmax*(Nmax+2)];
...
if verbose
    figure(201011)
    imagesc(log10(abs(Q)+1e-15));colorbar
    figure(201012)
    imagesc(log10(abs(RgQ)+1e-15));colorbar
