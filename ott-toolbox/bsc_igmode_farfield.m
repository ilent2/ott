function [nn,mm,a,b] = bsc_igmode_farfield( anglez,ig_mode,varargin )
% bsc_hgmode_farfield.m
% Uses a line integral method of LG beams to find the
% spherical harmonic expansion of a Ince-Gaussian laser beam.
%
% Usage:
% [n,m,a,b] = bsc_igmode_farfield( angle,ig_mode );
%
% calculates the expansion coefficients for a 'ig_mode' ([p,m,xi]) x-polarised
% beam converging with a cone of 'angle' (in degrees)
%
% OR
%
% [n,m,a,b] = bsc_igmode_farfield( angle,ig_mode,(optional))
%
...
paraxial_order=ig_mode(1);

[modeweights,lgindex,igindex]=genLG2IG(paraxial_order,ig_mode(3));

[m_,s_]=iglookup(paraxial_order,igindex);

[row,col]=find(and(m_==ig_mode(2),s_==eo),1);
